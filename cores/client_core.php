<?php
 ...
 Class client_core{
  ...
  public function signin_signup_ar ()
  {  
  require_once('views/signin_signup_ar.php');}
  ...
  { 
   ...
   if ($password!=$password2) {
    ?>
      <script>
       Javascript:alert('check your password!')
       document.location.replace("?core=client&action=signin_signup_ar");
       </script>
    <?php 
	} 
	else 
  ...
  {$password = $_POST['password'];
   ...
   $client= Client::verif($unique_id,$password);
    if(empty($client))
    {
        ?>
      <script>
       Javascript:alert('تحقق من الهوية أو كلمة المرور الخاصة بك!')
       document.location.replace("?core=client&action=signin_signup_ar");
       </script>
    <?php 
    }
