<?php
 ...
 Class user_core{
  ...
  public function signin_signup_user_ar ()
  {  
  require_once('views/signin_signup_user_ar.php');}

    public function logout ()
  {  session_unset();
   session_destroy();
  	require_once('views/signin_signup_user.php');}
        public function logout_ar ()
  {  session_unset();
   session_destroy();
    require_once('views/signin_signup_user_ar.php');}
...
public function  updateRej(){
  ...
  { 
   ...
   if ($password!=$password2) {
    ...
    public function sign_up_user_ar ()
  { 
   $name = $_POST['Name'];
   $LastName = $_POST['LastName'];
   $username = $_POST['username'];
   ...
   if ($password!=$password2) {
    ?>
      <script>
       Javascript:alert('check your password!')
       document.location.replace("?core=user&action=signin_signup_user_ar");
       </script>
    <?php 
  } 
  else 
  ...
  public function sign_in_user_ar()
  {$password = $_POST['password'];
   $username = $_POST['username'];
   $user= User::verif($username,$password);
    if(empty($user))
    ...
    <?php 
                ...
                $_SESSION['unique_id'] = $user->getUnique_id();
                $table= Auths::all();
   require_once('views/userdash_ar.php');}
}
